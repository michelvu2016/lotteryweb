...
@Component
public class FutureTransactionResultManager<T extends Future<?>> {

	private  Map<String, T> dataStore = Collections.synchronizedMap(new HashMap<String, T>());
	
	public FutureTransactionResultManager() {
	 ...
	 * @throws Exception
	 */
	public Object retrieveData(final String dataKey) throws Exception {
		
		return  dataStore.remove(dataKey).get();
	}	
